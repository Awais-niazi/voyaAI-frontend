...
function ResultsContent() {
  ...
  return (
    <div className="flex min-h-[calc(100vh-60px)]">
      <div className="w-[320px] bg-white border-r border-black/5 p-6 overflow-y-auto sticky top-[60px] max-h-[calc(100vh-60px)]">
          ...
          <>
            <div className="grid grid-cols-4 gap-3 mb-6">
              {[
                ...
                <div key={s.label} className="bg-[#f4f2ee] rounded-lg p-3.5">
                  <p className="text-xs text-[#8f8c85] uppercase tracking-wide mb-1">{s.label}</p>
                  <p className="text-xl font-medium text-[#1c1b19]">{s.value}</p>
                </div>
              ))}
            </div>

            <div className="mb-6">
              <h2 className="font-serif text-2xl font-semibold text-[#1c1b19] mb-1">
            ...
            <div className="relative pl-8">
              <div className="absolute left-[9px] top-3 bottom-3 w-0.5 bg-black/5" />
              <div className="space-y-4">
                {activeDay.activities.map((act: Activity, i: number) => (
                  <div key={i} className="relative">
                    <div className="absolute -left-8 top-3.5 w-4 h-4 rounded-full bg-white border-2 border-[#2d9e82]" />
                    <div className="bg-white rounded-xl border border-black/5 p-5 hover:border-black/10 hover:shadow-sm transition-all">
                      <p className="text-xs font-medium text-[#2d9e82] mb-1 tracking-wide">
                        {act.time} {act.duration && `· ${act.duration}`}
                      </p>
                      <h3 className="font-medium text-[#1c1b19] mb-1">{act.name}</h3>
